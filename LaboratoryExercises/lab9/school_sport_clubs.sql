...
DELIMITER |
CREATE TRIGGER ex1 BEFORE DELETE ON salarypayments
FOR EACH ROW
BEGIN
...
DELIMITER ;
		...
		/*  2 */
# TRUNCATE TABLE изтрива всичките записи от таблицата
# TRUNCATE TABLE не активира тригери и за това
# трябва да изтрием полетата от таблицата на ръка
# Без TRUNCATE id-тата на coaches започват от 25 
TRUNCATE TABLE salarypayments;
INSERT INTO salarypayments (coach_id, month, year,
							 salaryAmount, dateOfPayment)
SELECT old_coach_id, old_month, old_year,
old_salaryAmount, old_dateOfPayment
...
DELIMITER |
CREATE TRIGGER ex3 BEFORE INSERT ON student_sport
FOR EACH ROW
BEGIN
SET @count = 
 (SELECT count(*)
FROM student_sport
WHERE student_sport.student_id = NEW.student_id
GROUP BY student_id	 );
IF ( @count > 1)
	THEN 
		SET @msg = concat("Warning, group count over 2 -> ", @count+1); 
...
INSERT INTO `school_sport_clubs`.`student_sport` (`student_id`, `sportGroup_id`) VALUES ('1', '2');

			/* 4 */ 
DROP VIEW IF EXISTS ex4;
CREATE VIEW ex4
AS 
SELECT students.name, count(student_sport.student_id) as groups_count
FROM students
JOIN student_sport
...
DELIMITER |
CREATE PROCEDURE ex5(IN coach_name VARCHAR(255))
BEGIN
SELECT coaches.name as coach_name,
students.name, sportgroups.id, sports.name
FROM students
JOIN student_sport
ON students.id = student_sport.student_id
JOIN sportgroups
...
|
DELIMITER ;
CALL ex5 ('Ivan Todorov Petkov');
select * from coaches;

		/* 6 */
DROP PROCEDURE IF EXISTS ex6;
DELIMITER |
CREATE PROCEDURE ex6( IN sport_name VARCHAR(255))
BEGIN
SELECT sports.name, coaches.name, sportgroups.location, 
		sportgroups.hourOfTraining, sportgroups.dayOfWeek
FROM coaches
JOIN sportgroups
ON coaches.id = sportgroups.coach_id
JOIN sports 
