...
DELIMITER |
CREATE PROCEDURE ex1(IN coach_name VARCHAR (255) )
BEGIN
SELECT  sports.name,sportgroups.location, sportgroups.hourOfTraining,
		sportgroups.dayOfWeek, coaches.name,students.phone
FROM students
JOIN student_sport
...
|
DELIMITER ;
SET @coach = 'Ivan Todorov Petkov';
CALL ex1(@coach);

			/*2*/
DROP PROCEDURE IF EXISTS ex2;
DELIMITER |
CREATE PROCEDURE ex2(IN id int)
BEGIN
SELECT  students.name as student_name, students.id as st_id,
	   sports.name as sport_name,coaches.name as coach_name
FROM students
JOIN student_sport
ON students.id = student_sport.student_id
JOIN sportgroups
...
CALL ex2(@student_id);
			...
			/*3*/
# !!!!!!! YEAR ВИНАГИ СЕ ПОДАВА ПО VARCHAR
# !!!! МОГАТ ДА СЕ ПОДАВАТ САМО nteger, decimal, 
#      floating-point, binary или nonbinary string, NULL
DROP PROCEDURE IF EXISTS ex3;
DELIMITER |
CREATE PROCEDURE ex3 (in student_name VARCHAR(255), 
					  in payment_year VARCHAR(4))
BEGIN
...
AND taxespayments.year = payment_year
GROUP BY taxespayments.student_id;
END
|
DELIMITER ;

call ex3("Iliyan Ivanov",2023);

			/*4*/
DROP PROCEDURE IF EXISTS ex4;
DELIMITER |
CREATE PROCEDURE ex4(IN coach_name VARCHAR(255))
BEGIN
...
WHERE coaches.name = coach_name
GROUP BY sportgroups.coach_id;
SET group_count = IFNULL(group_count, 0); 
SELECT IF( group_count = 0, "NO GROUPS", group_count) 
as groups_count;
END
|
DELIMITER ;
CALL ex4('Slavi Petkov Petkov');
