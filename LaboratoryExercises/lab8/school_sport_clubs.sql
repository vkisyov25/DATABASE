...
INSERT INTO `school_sport_clubs`.`coach_work` (`coach_id`, `group_id`, `number_of_hours`, `date`) VALUES ('2', '3', '2', '2023-04-09 08:45:55');
			...
			/*1*/
DROP VIEW IF EXISTS ex_1_view;
CREATE VIEW ex_1_view 
AS
SELECT DISTINCT coaches.name as coach_name, concat(sportgroups.id, "-", sportgroups.location) as groupInfo,
sports.name, salarypayments.year, salarypayments.month, salarypayments.salaryAmount
FROM coaches
JOIN sportgroups 
...
SELECT  * From ex_1_view;

			/*2*/
DROP PROCEDURE IF EXISTS lab_8_ex2;
DELIMITER |
CREATE PROCEDURE lab_8_ex2 ()
BEGIN 
SELECT students.name,count(students.id) as grouops_count
FROM students
JOIN student_sport
...
CALL lab_8_ex2();
			...
			/*3*/
# "= NULL " не връща нищо, за да проверим нещо дали е null
# ползваме функциятат ISNULL(); 
DROP PROCEDURE IF EXISTS lab_8_ex3;
DELIMITER |
CREATE PROCEDURE lab_8_ex3()
BEGIN
SELECT coaches.name, sportgroups.id
FROM coaches
LEFT JOIN sportgroups 
ON coaches.id = sportgroups.coach_id
WHERE ISNULL(sportgroups.coach_id);
END
|
DELIMITER ;
