...
FROM students s
JOIN student_sport ON s.id = student_sport.student_id
...
FROM students s JOIN student_sport 
ON s.id = student_sport.student_id
JOIN sportgroups 
...
create view four 
AS
SELECT month, SUM(paymentAmount) as sum
FROM students s JOIN student_sport 
ON s.id = student_sport.student_id
JOIN sportgroups 
ON student_sport.sportgroup_id = sportgroups.id
...
ON sportgroups.coach_id = coaches.id
JOIN taxespayments 
ON student_sport.student_id = taxespayments.student_id
#WHERE coaches.name = '7509041245'
GROUP BY `month`
having sum>700;
